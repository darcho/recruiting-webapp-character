...
const PartySkillCheckCard = () => {
  const { getCharacterWithHighSkill } = useCharacter();
  const [skill, setSkill] = useState<string>(SKILL_LIST[0].name);
  const [character, index] = getCharacterWithHighSkill(skill);
  ...
  return (
    <Card>
      <CardTitle>Party Skill Check</CardTitle>
      <CardContent>
        {dice ? (
          <Stack>
            <div>Character {index + 1}</div>
            <div>
...
export default PartySkillCheckCard;